...
public class NewsAdapter extends BaseAdapter {
    ...
    private static final int ITEM_TYPT_WITH_1_IMAGE = 0;
    private static final int ITEM_TYPT_WITH_2_IMAGE = 1;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
       //列表项数据
        ...
        if(itemViewType == ITEM_TYPT_WITH_1_IMAGE){ //第一种类型的列表项
        ...
        } else if (itemViewType == ITEM_TYPT_WITH_2_IMAGE){  //第二种类型的列表项
            if(convertView == null){
    ...
    public int getItemViewType(int position) {
        ...
        if (item.getImgextra() == null || item.getImgextra().size() == 0){
            //只有一张图片的item
            return ITEM_TYPT_WITH_1_IMAGE;
        } else {
            //item中有三张图片
            return ITEM_TYPT_WITH_2_IMAGE;
        }
    }
